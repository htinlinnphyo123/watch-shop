...
const openStock = item => { stock.reset(); stock.clearErrors(); stockItem.value = item; };
const stockAfter = computed(() => Number(stockItem.value?.available_items || 0) + (stock.operation === 'add' ? 1 : -1) * Number(stock.quantity || 0));
const saveStock = () => stock.post(route('accessories.stock', stockItem.value.id), { preserveScroll: true, onSuccess: () => stockItem.value = null });
...
<template>
    ...
    <AdminLayout>
        ...
        <AccessoryDialog :show="!!stockItem" title="Adjust stock" :description="stockItem?.name" :busy="stock.processing" @close="stockItem = null">
            <form @submit.prevent="saveStock" class="accessories"><div class="space-y-5 p-6"><div class="grid grid-cols-2 gap-3 rounded-xl bg-gray-50 p-4"><div><p class="hint">Available now</p><p class="mt-1 text-2xl font-semibold">{{ stockItem?.available_items }}</p></div><div class="border-l border-gray-200 pl-5"><p class="hint">After adjustment</p><p class="mt-1 text-2xl font-semibold" :class="stockAfter < 0 ? 'text-red-600' : 'text-gold-700'">{{ stockAfter }}</p></div></div><div class="grid grid-cols-2 gap-3"><label class="label">Action<select v-model="stock.operation" class="field"><option value="add">Add stock</option><option value="remove">Remove stock</option></select></label><label class="label">Quantity<input v-model="stock.quantity" class="field" required type="number" min="1" :max="stock.operation === 'remove' ? Math.min(500, stockItem?.available_items || 0) : 500" /></label></div><p class="hint">New units receive a unique barcode automatically. Only available units can be removed. Sold and reserved units stay in your records.</p><p v-for="error in stock.errors" :key="error" class="text-sm text-red-600" role="alert">{{ error }}</p></div><footer class="dialog-footer"><button type="button" class="btn-secondary" :disabled="stock.processing" @click="stockItem = null">Cancel</button><button :disabled="stock.processing || stockAfter < 0" class="btn-primary">{{ stock.processing ? 'Updating…' : 'Update stock' }}</button></footer></form>
        </AccessoryDialog>
